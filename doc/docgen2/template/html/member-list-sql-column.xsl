...
<xsl:stylesheet
    ...
    <xsl:value-of select="ext:let('content-node', ext:caller('content-node'))" />
        ...
        <tr><td colspan="3" class="thdr"><b><xsl:value-of select="ext:caller('group-name')" /></b></td></tr>
        <xsl:choose>
    ...
    <xsl:template match="member">
            <xsl:if test="./@exclude-from-list='false'" >
                ...
                <td class="tmain_nw"><code><xsl:value-of select="./declaration[./@language='sql']/@return" /></code></td>
