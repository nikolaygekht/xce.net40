...
<!-- detect language -->
<xsl:value-of select="ext:let('language', 'SQL')" />
<!-- build body depend on the member type -->
<table class="decl">
 <tr><td><xsl:value-of select="ext:get('language')" /></td></tr>
 <tr><td valign="top" colspan="2"><code><xsl:value-of select="concat(ext:get('prefix'), ext:get('name'), ext:get('name-suffix'), '&amp;nbsp;')" disable-output-escaping="yes" />
                            <xsl:value-of select="concat(ext:get('return'), ext:get('suffix'))" disable-output-escaping="yes" /></code></td></tr>
 <xsl:if test="count(./body/*)>0">
    <xsl:value-of select="ext:let('curr-item', .)" />
    <tr><td width="1%"></td><td><code><xsl:value-of select="ext:call('write-description.xsl', /)" disable-output-escaping="yes" /></code></td></tr>
 </xsl:if>
</table>
    </xsl:template>
</xsl:stylesheet>
