...
<xsl:choose>
    <!-- method -->
    <xsl:when test="../@type='method' and ./@custom!='sub'">
        <xsl:value-of select="ext:let('decl-body', concat(ext:get('prefix'), 'Function &lt;b&gt;', ext:get('name'), ext:get('name-suffix'), '&lt;/b&gt; (', ext:get('params'), ')', ' As ', ext:get('return'), ' ', ext:get('suffix'))) " />
    </xsl:when>
    <xsl:when test="../@type='method' and ./@custom='sub'">
        <xsl:value-of select="ext:let('decl-body', concat(ext:get('prefix'), 'Sub &lt;b&gt;', ext:get('name'), ext:get('name-suffix'), '&lt;/b&gt; (', ext:get('params'), ')', ext:get('suffix'))) " />
    </xsl:when>
    <!-- field or property -->
    <xsl:otherwise>
        <xsl:choose>
            ...
            <xsl:when test="string-length(ext:get('params')) > 0">
                <xsl:value-of select="ext:let('decl-body', concat(ext:get('prefix'), '&lt;b&gt;', ext:get('name'), ext:get('name-suffix'), '&lt;/b&gt; (', ext:get('params'), ')', ' As ', ext:get('return'), ' ', ext:get('suffix'))) " />
            </xsl:when>
            <!-- indexed property/field -->
            <xsl:otherwise>
                <xsl:value-of select="ext:let('decl-body', concat(ext:get('prefix'), '&lt;b&gt;', ext:get('name'), ext:get('name-suffix'), '&lt;/b&gt;', ' As ', ext:get('return'), ' ', ext:get('suffix'))) " />
            </xsl:otherwise>
        </xsl:choose>
    </xsl:otherwise>
</xsl:choose>
