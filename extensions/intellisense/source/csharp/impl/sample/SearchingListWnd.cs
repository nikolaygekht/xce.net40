...
{
    class SearchingListWindow : Window, IEnumerable<DialogItemListBoxString>
    {
        ...
        public override void OnKeyPressed(int scanCode, char character, bool shift, bool ctrl, bool alt)
        {
            ...
            else if (!ctrl && !alt && (character >= ' '))
            {
                ...
                for (int i = 0; i < mItems.Count; i++)
                {
                    ...
                    if ((idx >= 0 && !mStartOnlySearch) || (idx >= 0 && idx <= mStartMax && mStartOnlySearch))
                    {
                        mCurSel = i;
                        EnsureVisible(mCurSel);
