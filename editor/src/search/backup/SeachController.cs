                ...
                case TextWindowBlock.Stream:
                    {
                        ...
                        if (lineEnd < w.BlockLineStart && lineLength > 0)
                        {
                            //region is before the block
                            w._BlockLineStart  -= lineLength;
                            w._BlockLineEnd -= lineLength;
                        }
                        else if (lineStart > w.BlockLineStart && lineEnd < w.BlockLineEnd && lineLength > 0)
                        {
                            //region is inside the multi-line
                            w._BlockLineEnd -= lineLength;
                        }
                        else if ((lineStart < w.BlockLineStart || (lineStart == w.BlockLineStart && columnStart <= w.BlockColumnStart)) &&
                                 (lineEnd > w.BlockLineEnd || (lineEnd == w.BlockLineEnd && columnEnd >= w.BlockColumnEnd)))
                        {
                            //region is outside the block
                            w.DeselectBlock();
                        }
                        else if (lineLength == 0 && lineStart == w.BlockLineStart && lineStart == w.BlockLineEnd)
                        {
                            //handle one-line block and one-line region
                            if (columnEnd < w.BlockColumnStart)
                            {
                                //one-line region is completely before the block
                                w._BlockColumnStart -= columnEnd - columnStart;
                                w._BlockColumnEnd -= columnEnd - columnStart;
                            }
                            else if (columnStart <
                        }
                        else
